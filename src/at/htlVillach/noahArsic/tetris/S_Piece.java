...
public class S_Piece extends GameObject {

    public S_Piece(Game game) {
        super(5, game);
        pieceHeight = 2;
        pieceWidth = 3;
        checkForGameOver();
    ...
    public void placePieceInField() {
        switch (rotation) {
            ...
            case 1 -> {
                ...
                for (int i = y + 1; i < y + 3; i++) FieldHandler.setPlace(x + 2, i, intId);
            }
            case 2 -> {
                for (int i = x + 1; i < x + 3; i++) FieldHandler.setPlace(i, y + 1, intId);
                for (int i = x; i < x + 2; i++) FieldHandler.setPlace(i, y + 2, intId);
            }
            case 3 -> {
